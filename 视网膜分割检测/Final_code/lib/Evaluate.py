...
class Evaluate():
    ...
    def Dice(self):
        # intersection
        intersection = self.output * self.target
        num = 2 * np.sum(intersection)
        den = np.sum(self.output) + np.sum(self.target)
        dice = num / den
        return dice
    ...
    def bac(self):
        y_pre = deepcopy(self.output)
        y_pre[y_pre >= 0.5] = 1
        y_pre[y_pre < 0.5] = 0
        return balanced_accuracy_score(self.target, y_pre)
    ...
    def save_all_result(self, plot_curve=True, save_name=None):
        ...
        if save_name is not None:
            ...
            file_perf.write("Dice" + str(Dice)
                            + "\nBAC" + str(Bac)
                            + "\nAUC ROC curve: " + str(AUC_ROC)
            ...
            file_perf.close()
        return OrderedDict([("Dice", Dice), ("BAC", Bac)])
